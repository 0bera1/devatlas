...
export const DIAGRAM_REPOSITORY: unique symbol = Symbol('DIAGRAM_REPOSITORY');
...
export interface IDiagramRepository {
  ...
  updateDiagramPatchByIdAndOwnerId(
    id: string,
    ownerId: string,
  ...
  selectPublicRelatedDiagramsBySharedNodeLabels(
    sourceDiagramId: string,
    take: number,
  ): Promise<DiagramSummary[]>;
  insertDiagramCollaborator(diagramId: string, userId: string): Promise<void>;
  deleteDiagramCollaborator(
    diagramId: string,
    userId: string,
  ): Promise<boolean>;
  selectDiagramCollaborators(
    diagramId: string,
