...
export const seedDocuments: SeedDocumentInput[] = [
  {
    ...
    summary:
      'Resource naming, HTTP verbs, status codes, pagination, versioning and idempotency for production APIs.',
    sortOrder: 1,
    content: bilingualSection(
      'REST API Best Practices',
      `**Resource-oriented URLs** use nouns, not verbs: \`GET /users/{id}\`, not \`GET /getUser\`.
...
**HATEOAS (optional)** — embed \`_links\` for discoverability in public platforms.`,
      'REST API En İyi Uygulamalar',
      `**Kaynak odaklı URL'ler** fiil değil isim kullanır: \`GET /users/{id}\`.
...
**İdempotans** — PUT/DELETE doğası gereği idempotent; ödeme POST'larında \`Idempotency-Key\` kullanın.`,
  ...
  },
  {
    ...
    summary:
      'When to choose bidirectional WebSockets vs one-way SSE for real-time features.',
    sortOrder: 2,
    content: bilingualSection(
      'WebSocket vs SSE',
      `**WebSocket** — full-duplex, binary + text, ideal for chat, collaborative editors, gaming, and low-latency trading. Requires connection management, heartbeats, and horizontal scaling via pub/sub (Redis, NATS).
...
**Choose SSE** when you only push updates and want simpler infra (standard load balancers, no upgrade handshake).`,
      'WebSocket vs SSE',
      `**WebSocket** — çift yönlü, düşük gecikme; sohbet, ortak düzenleme ve oyunlar için uygundur. Kalp atışı ve pub/sub ile ölçekleme gerekir.
...
**SSE seçin** yalnızca sunucu itecek ve proxy uyumluluğu önemliyse.`,
  ...
  },
  {
    ...
    summary:
      'Edge routing, auth termination, rate limiting, aggregation and BFF at the gateway layer.',
    sortOrder: 3,
    content: bilingualSection(
      'API Gateway',
      `An **API Gateway** sits at the edge: TLS termination, routing, authentication, rate limiting, request/response transformation, and observability.
...
Popular stacks: Kong, NGINX, Envoy, AWS API Gateway, Azure APIM.`,
      'API Gateway',
      `**API Gateway** kenarda durur: TLS, yönlendirme, kimlik doğrulama, hız sınırlama ve dönüşüm.
...
**Devre kesici** — bozulan servislerde hızlı hata döndürür.`,
  ...
  },
  {
    ...
    summary:
      'B-tree indexes, composite keys, covering indexes, and when indexes hurt write throughput.',
    sortOrder: 4,
    content: bilingualSection(
      'Indexes in Relational Databases',
      `An **index** is a sorted data structure (usually B-tree) that speeds up reads at the cost of extra storage and slower writes.
...
**Avoid** over-indexing: each INSERT/UPDATE maintains index pages. Analyze with \`EXPLAIN\` / execution plans.`,
      'Veritabanı Mühendisliği: İndeksler',
      `**İndeks** okumayı hızlandırır, yazmayı ve diski yükler; genelde B-tree yapısıdır.
...
**Aşırı indeks** yazma performansını düşürür; \`EXPLAIN\` ile doğrulayın.`,
  ...
  },
  {
    ...
    summary:
      'Caching, session store, pub/sub, distributed locks and data structure selection.',
    sortOrder: 5,
    content: `# Redis
...
**Data structures** — Strings, Hashes (objects), Sets (tags), Sorted Sets (leaderboards), HyperLogLog (cardinality).`,
  },
  {
    ...
    summary:
      'Tokens, components, documentation and visual regression in a design system workflow.',
    sortOrder: 6,
    content: bilingualSection(
      'Design System with Storybook',
      `A **design system** unifies color, typography, spacing tokens and reusable UI primitives.
...
**Governance** — versioning, deprecation policy, and contribution guidelines keep teams aligned.`,
      'Tasarım Sistemi ve Storybook',
      `**Tasarım sistemi** renk, tipografi ve spacing token'larını standartlaştırır.
...
**Yönetişim** — sürümleme ve katkı kuralları ekipleri hizalar.`,
  ...
  },
  {
    ...
    summary:
      'Local UI state, server cache, global store, URL state and when to combine them.',
    sortOrder: 7,
    content: bilingualSection(
      'State Management',
      `**Local state** — form inputs, toggles; keep close to components (\`useState\`).
...
**Rule of thumb** — default local + server cache; add global store only for truly shared client concerns.`,
      'Durum Yönetimi Stratejileri',
      `**Yerel durum** — form ve UI geçici değerleri bileşende tutun.
...
**Özet** — önce yerel + sunucu önbelleği; global store son çare.`,
  ...
  },
  {
    ...
    summary:
      'Independent deployable frontends, module federation, routing and shared design tokens.',
    sortOrder: 8,
    content: bilingualSection(
      'Micro Frontend',
      `**Micro frontends** split the UI into independently deployable apps (by domain or team).
...
**Risks** — dependency duplication, inconsistent UX, and harder E2E — mitigate with shared component library.`,
      'Micro Frontend',
      `**Micro frontend** arayüzü bağımsız deploy edilen parçalara böler.
...
**Riskler** — bundle tekrarı ve tutarsız UX; paylaşılan kütüphane ile azaltın.`,
  ...
  },
  {
    ...
    summary:
      'Single deployable SPA with feature modules — contrast with micro frontend distribution.',
    sortOrder: 9,
    content: bilingualSection(
      'Macro Frontend',
      `**Macro frontend** keeps one build and deploy but organizes code in feature modules (folders, lazy routes, bounded contexts).
...
**Lazy loading** — route-based code splitting still applies inside the monolith.`,
      'Macro Frontend',
      `**Macro frontend** tek deploy ile feature modülleri ayırır.
...
**Lazy load** — monolit içinde rota bazlı bölme devam eder.`,
  ...
  },
  {
    ...
    summary:
      'Route-based splitting, dynamic import, images and prefetch strategies.',
    sortOrder: 10,
